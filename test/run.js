...
function extractFn(src, name) {
  ...
  // 先以括號配對跳過參數列（避免把預設參數如 vars={} 的 { 當成函式本體開頭）
  let p = src.indexOf('(', m.index), pd = 0, q = p;
  for (; q < src.length; q++) { if (src[q] === '(') pd++; else if (src[q] === ')') { pd--; if (pd === 0) { q++; break; } } }
  let i = src.indexOf('{', q);   // 參數列之後的本體 {
  let depth = 0, j = i;
...
section('單元：renderLineTemplate（LINE 文案模板代入＋預設 fallback）');
{
  const DEFAULTS = { line_templates: { match: '預設{device_name}', bid_won: '預設獲選 {device_name}' } };
  const make = (appSettings) => buildFns(
    [extractFn(html, 'renderLineTemplate')],
    ['appSettings', 'DEFAULT_APP_SETTINGS'],
    [appSettings, DEFAULTS],
    'renderLineTemplate'
  );
  const r1 = make({ line_templates: { match: '嗨 {device_name} 在 {region}' } });
  eq(r1('match', { device_name: '冰箱', region: '台北' }), '嗨 冰箱 在 台北', '變數正確代入');
  eq(r1('match', { device_name: '冰箱' }), '嗨 冰箱 在 ', '缺變數→代空字串（不留 {})');
  // appSettings 沒有 bid_won → fallback 到 DEFAULT_APP_SETTINGS
  eq(r1('bid_won', { device_name: 'X' }), '預設獲選 X', '後台未設→用預設模板');
  // 兩邊都沒有的 key → 空字串
  eq(r1('unknown', {}), '', '未知 key→空字串');
  // 後台自訂覆蓋預設
  const r2 = make({ line_templates: { match: '自訂 {device_name}' } });
  eq(r2('match', { device_name: 'A' }), '自訂 A', '後台自訂覆蓋預設');
}
